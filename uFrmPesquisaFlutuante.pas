...
type
  TFluentFactory = function: TValue;
  TFluentMethodMatch = (fmmExact, fmmPrefix);

  TFluentRegistration = record
    FluentClass: TClass;
    Factory: TFluentFactory;
    FactoryName: string;
  end;
...
procedure RegisterFluentClass(AClass: TClass;
  const AFactory: TFluentFactory; const AFactoryName: string);
procedure IgnoreFluentMethod(const APattern: string;
  AMatch: TFluentMethodMatch = fmmExact);

var
  frmPesquisaFlutuante: TfrmPesquisaFlutuante;
  GFormsJSON: string = '';
{$IFDEF DEBUG}
  GFluentDebugEnabled: Boolean = True;
{$ELSE}
  GFluentDebugEnabled: Boolean = False;
{$ENDIF}
...
var
  GFluentRegistrations: TList<TFluentRegistration>;
  GIgnoredFluentMethods: TDictionary<string, TFluentMethodMatch>;
...
var
  Index: Integer;
  Registration: TFluentRegistration;
begin
  if AClass = nil then
    raise EArgumentNilException.Create('AClass');
  if not Assigned(AFactory) then
    raise EArgumentNilException.Create('AFactory');

  Registration.FluentClass := AClass;
  Registration.Factory := AFactory;
  Registration.FactoryName := AFactoryName;

  for Index := 0 to GFluentRegistrations.Count - 1 do
    if GFluentRegistrations[Index].FluentClass = AClass then
    begin
      GFluentRegistrations[Index] := Registration;
      Exit;
    end;

  GFluentRegistrations.Add(Registration);
end;

procedure IgnoreFluentMethod(const APattern: string;
  AMatch: TFluentMethodMatch);
begin
  if Trim(APattern) = '' then
    raise EArgumentException.Create('APattern must not be empty');

  GIgnoredFluentMethods.AddOrSetValue(APattern, AMatch);
end;

function IsFluentMethodIgnored(const AMethodName: string): Boolean;
var
  Rule: TPair<string, TFluentMethodMatch>;
begin
  for Rule in GIgnoredFluentMethods do
    case Rule.Value of
      fmmExact:
        if CompareText(AMethodName, Rule.Key) = 0 then
          Exit(True);
      fmmPrefix:
        if StartsText(Rule.Key, AMethodName) then
          Exit(True);
    end;
...
function GetFluentRegistrations: TArray<TFluentRegistration>;
begin
  Result := GFluentRegistrations.ToArray;
end;

function TryGetRegisteredFluentFactory(const AClassName: string;
  out AClass: TClass; out AFactory: TFluentFactory;
  out AFactoryName: string): Boolean;
var
  Registration: TFluentRegistration;
begin
  AClass := nil;
  ...
  for Registration in GFluentRegistrations do
    if SameText(Registration.FluentClass.ClassName, AClassName) then
    begin
      AClass := Registration.FluentClass;
      AFactory := Registration.Factory;
      AFactoryName := Registration.FactoryName;
      Exit(True);
    end;
...
var
  ...
  Cls: TClass;
  Caption: string;
  FluentRegistration: TFluentRegistration;
begin
  ...
  try
    ...
    for FluentRegistration in GetFluentRegistrations do
    begin
      Cls := FluentRegistration.FluentClass;
      if Cls = nil then
        Continue;

      Obj := TJSONObject.Create;
      Obj.AddPair('cls', Cls.ClassName);
      Obj.AddPair('caption', FriendlyNameFromClass(Cls.ClassName));
...
var
  ...
  FactoryName: string;
  FluentPath: string;
  DebugPathShown: Boolean;

  procedure ShowDebugPath;
  begin
    if GFluentDebugEnabled and not DebugPathShown then
    begin
      DebugPathShown := True;
      MessageDlg('Fluent path:' + sLineBreak + FluentPath,
        mtInformation, [mbOK], 0);
    end;
  end;

  function IsTerminalMethod(AMethod: TRttiMethod): Boolean;
  begin
    Result := ContainsText(AMethod.Name, 'Show');
  end;
  ...
  begin
    Result :=
      ...
      (not IsFluentMethodIgnored(AMethod.Name)) and
      (IsTerminalMethod(AMethod) or
       (Assigned(AMethod.ReturnType) and Assigned(CurType) and
        (AMethod.ReturnType = CurType)));
  end;

begin
  Ctx := TRttiContext.Create;
  FluentPath := AClassName;
  DebugPathShown := False;

  if not TryGetRegisteredFluentFactory(AClassName, FluentClass,
    FluentFactory, FactoryName) then
  begin
    MessageDlg('Classe "' + AClassName + '" não segue a convenção fluente ' +
      '(fábrica não registrada).',
      mtWarning, [mbOK], 0);
    Exit;
  end;

  FluentPath := FluentClass.ClassName + '->' + FactoryName;

  try
    CurValue := FluentFactory();
  except
    on E: Exception do
    begin
      MessageDlg('Erro ao criar ' + FluentClass.ClassName + ': ' + E.Message,
        mtError, [mbOK], 0);
      Exit;
    end;
  end;
  ...
  begin
    MessageDlg('A interface "' + CurType.Name + '" retornada pela fábrica não expõe RTTI de métodos. ' +
  ...
  repeat
    ...
    begin
      ...
      Executed := True;
      FluentPath := FluentPath + '->' + Meth.Name;

      if IsTerminalMethod(Meth) then
      begin
        ShowDebugPath;
        Exit;
      end;
...
procedure RodarAsync(const AProc: TThreadProcedure);
begin
...
begin
  ...
  Core.AddWebResourceRequestedFilterWithRequestSourceKinds(
    '*',
...
initialization
  GFluentRegistrations := TList<TFluentRegistration>.Create;
  GIgnoredFluentMethods := TDictionary<string, TFluentMethodMatch>.Create;
  IgnoreFluentMethod('Get', fmmPrefix);
  IgnoreFluentMethod('Set', fmmPrefix);
  IgnoreFluentMethod('Free');
  IgnoreFluentMethod('DisposeOf');
  IgnoreFluentMethod('ToString');
  IgnoreFluentMethod('Equals');

finalization
  GIgnoredFluentMethods.Free;
  GFluentRegistrations.Free;
